...
for color_name, bgr in colors.items():
    for shape in shapes:
        for _ in range(10):
            ...
            mean_color = cv2.mean(img)[:3] #середнє значення кольорів шукаєм
            features = [mean_color[0], mean_color[1], mean_color[2]] #середнє значення по b, g, r
            # числові значення по яким буде навчатись наша модель
...
test_img = generate_image((255, 15, 20), 'square')
