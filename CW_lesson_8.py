...
face_net = cv2.dnn.readNetFromCaffe('dnn/deploy.prototxt', 'dnn/res.caffemodel')
...
#__________________dnn______________________
    (h, w) = frame.shape[:2]
    blob = cv2.dnn.blobFromImage(frame, 1.0, (300, 300), (104.0, 177.0, 123.0)) #формат

    face_net.setInput(blob)
    detections = face_net.forward()

    for i in range(detections.shape[2]):
        confidence = detections[0, 0, i, 2] #детектуємо наскільки ми впевнені в тому шо то обличчя

        if confidence > 0.5: #confidence - впевненість
...
#_________________cascade_____________________
