import cv2
import numpy as np
image = cv2.imread('images/photo.jpg')
print(image.shape)
image = cv2.resize(image, (image.shape[1] // 2, image.shape[0] // 2))
image =  cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
image = cv2.Canny(image, 100, 180)
carnal = np.ones((2, 2), np.uint8)
image = cv2.dilate(image, carnal, iterations=1)
image = cv2.erode(image, carnal, iterations=1)

image1 = cv2.imread('images/photo1.jpg')
print(image1.shape)
image1 = cv2.resize(image1, (image1.shape[1] // 3, image1.shape[0] // 3))
image1 =  cv2.cvtColor(image1, cv2.COLOR_BGR2GRAY)
image1 = cv2.Canny(image1, 100, 180)
carnal = np.ones((5, 5), np.uint8)
image1 = cv2.dilate(image1, carnal, iterations=1)
image1 = cv2.erode(image1, carnal, iterations=1)

cv2.imwrite("photo.jpg", image)
cv2.imwrite("photo1.jpg", image1)

cv2.imshow('image', image)
cv2.imshow('image', image1)
cv2.waitKey(0)
cv2.destroyAllWindows()